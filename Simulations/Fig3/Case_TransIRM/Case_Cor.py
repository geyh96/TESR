...
The_DATA_MARK = "idata_" + str(idx_data) + "_ideparture_" + str(ideparture) + "_numS_" + str(numS)
    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if loss_iepoch < loss_best_ifold:
                ...
                torch.save(net.state_dict(), os.path.join(args.save, "dCOR_net_idata_" +  The_DATA_MARK  + "Model_iloop_" +  str(ithres) + '_Best' + '_net.pt'))
##########################prediction
...
file_net_dict =  os.path.join(args.save,  "dCOR_net_idata_" + The_DATA_MARK  + "Model_iloop_" +  str(idx_best) + '_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim,outdim = 1)
    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if loss_iepoch < loss_best:
                ...
                torch.save(f1_pred_from_Rx.state_dict(), os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_iloop_"  + str(ithres) + '_Best'  + '_net.pt'))
...
file_net_dict = os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_iloop_" +str(idx_best2) + '_Best' + '_net.pt')
f1_pred_from_Rx = pred_from_Rx(args.latent_dim, outdim = 2)
...
import pickle
with open(os.path.join(args.save_pickle,  "Res_" + The_DATA_MARK  + "_Model_iloop_" +  str(idx_data) + '_Best' + '_result.pickle'), 'wb') as handle:
    pickle.dump(
        res
