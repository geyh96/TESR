...
The_DATA_MARK = "idata_" + str(idx_data) + "_idx_Target_" + str(idx_Target) + "_NS_" + str(NSource) + "_NT_" + str(NTarget) + "_P_" + str(P)
...
if 1:
    s = idx_Target
    DataT_train = Get_data_0101(NTarget,P,s=s)
    DataT_val = Get_data_0101(NTval,P,s=s)
    DataT_test = Get_data_0101(ntest,P,s=s)
...
for ithres in range(nthres1):
    ...
    net = Generator(xdim = P, ndim = args.latent_dim,outdim = 1)
    ...
    for epoch in range(args.nEpochs):
        ...
        for batch_idx, (X,Rx,Y,weight, setidx) in enumerate(Loader_train):
            ...
            for im in range(m):
                iim = im + 1
                idx_im = torch.nonzero(torch.eq(setidx, iim))[:,0]
                X_im = X[idx_im]
                Y_im = Y[idx_im]
                w, output_im = net(X_im)
                dummy_w = torch.nn.Parameter(torch.ones_like(output_im))
                lossi = MSEloss(output_im*dummy_w,Y_im)  

                penalty_im = grad(lossi,dummy_w ,create_graph=True)[0].pow(2).mean()
                sum_penalty = sum_penalty + penalty_im
                loss_allclass = loss_allclass + lossi
            loss_epoch = (1 - lambda_IRMloss) * loss_allclass/m + lambda_IRMloss * sum_penalty/m
        ...
        if epoch % 1==0:
            ...
            if epoch %10 == 0:
                print("the Val loss {:4f}".format(loss_iepoch))
            #############################################################################################
            if loss_iepoch < loss_best_ifold:
                ...
                torch.save(net.state_dict(), os.path.join(args.save, "dCOR_net_idata_" +  The_DATA_MARK  + "Model_iloop_" +  str(ithres) + '_n_Best' + '_net.pt'))
##########################prediction
...
file_net_dict =  os.path.join(args.save,  "dCOR_net_idata_" + The_DATA_MARK  + "Model_iloop_" +  str(idx_best) + '_n_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim,outdim = 1)
    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if loss_iepoch < loss_best:
                ...
                torch.save(f1_pred_from_Rx.state_dict(), os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_iloop_"  + str(ithres) + '_n_Best'  + '_net.pt'))
...
file_net_dict = os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_iloop_" +str(idx_best2) + '_n_Best' + '_net.pt')
f1_pred_from_Rx = pred_from_Rx(args.latent_dim, outdim = 2)
...
import pickle
with open(os.path.join(args.save_pickle,  "Res" + The_DATA_MARK  + "_Model_iloop_" +  str(idx_data) + '_n_Best' + '_result.pickle'), 'wb') as handle:
    pickle.dump(
        res
