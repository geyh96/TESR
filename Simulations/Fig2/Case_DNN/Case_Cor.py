...
for ithres in range(nthres):
    ...
    for epoch in range(args.nEpochs_pred):
        ...
        if loss_iepoch < loss_best_ifold:
            ...
            torch.save(net.state_dict(), os.path.join(args.save,   The_DATA_MARK  + "Model_iloop_" +  str(ithres) + '_n_Best' + '_net.pt'))
##########################prediction
idx_best = np.argmin(loss_eval)
file_net_dict =  os.path.join(args.save,   The_DATA_MARK  + "Model_iloop_" +  str(idx_best ) + '_n_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim)
...
import pickle
with open(os.path.join(args.save_pickle,  "Res" + The_DATA_MARK  + '_n_Best' + '.pickle'), 'wb') as handle:
