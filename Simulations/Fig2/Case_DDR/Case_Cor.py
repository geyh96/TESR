...
for epoch_2 in range(args.nEpochs_pred):
    ...
    if epoch_2 %2 ==0:
        ...
        if loss_iepoch < loss_best:
            ...
            torch.save(f1_pred_from_Rx.state_dict(), os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK + "Model_iloop_"  + str(idx_best) + '_net.pt'))
...
file_net_dict = os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_iloop_"  + str(idx_best) + '_net.pt')
...
import pickle
with open(os.path.join(args.save_pickle,  "Res" + The_DATA_MARK  + '_n_Best' + '_result.pickle'), 'wb') as handle:
    pickle.dump(
        res
