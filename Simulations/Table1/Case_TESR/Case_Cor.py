...
for ithres in range(nthres1):
    ...
    net = Generator(xdim = P, ndim = dim, outdim = 2)
    ...
    for epoch in range(args.nEpochs):
        ...
        for batch_idx, (X,Rx,Y,weight, setidx) in enumerate(Loader_train):
            ...
            D = torch.randn(Yi.shape[0], dim).to(device)
            w, _ = net(Xi)
...
for ithres in range(nthres2):
    ...
    net_Target = Generator(xdim = P, ndim = dim, outdim = 2)
    file_net_dict =  os.path.join(args.save, "dCOR_idata_" + The_DATA_MARK  + "Mloop_"  + str(idx_best) + '_net_Best' + '_net.pt')
...
net2 = Generator(xdim = P, ndim = dim,outdim =  2)
...
net = net.eval()
net2 = net2.eval()
f1_pred_from_Rx = pred_from_Rx(dim*2,outdim=2)
