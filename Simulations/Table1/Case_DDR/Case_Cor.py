...
for ithres in range(nthres):
    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if epoch % 10==0:
                print("On Val Dataset "+'\nEpoch {}:,Test set: dCor_loss: {:.4f},Rx_loss: {:.4f}'.format(epoch,dCor_loss,fit_Rx_loss))
            ###########################################################
...
f1_pred_from_Rx = pred_from_Rx(2*dim)
...
with open(os.path.join(args.save_pickle,  "dCOR_idata_result_" + The_DATA_MARK  + '_net_Best' + '_result.pickle'), 'wb') as handle:
    pickle.dump(
        # [mse_pred_REweightedmodel,mse_pred_REweightedmodel]
        res
