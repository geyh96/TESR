    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if loss_iepoch < loss_best_ifold:
                ...
                torch.save(net.state_dict(), os.path.join(args.save, "dCOR_net_idata_" +  The_DATA_MARK  + "Model_REweighted_iloop_" +  str(ithres) + '_net_Best' + '_net.pt'))
##########################prediction
...
file_net_dict =  os.path.join(args.save,  "dCOR_net_idata_" + The_DATA_MARK  + "Model_REweighted_iloop_" +  str(idx_best) + '_net_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim,outdim = 1)
...
for ithres in range(nthres):
    ...
    f1_pred_from_Rx = pred_from_Rx(args.latent_dim,outdim=1)
    ...
    for epoch in range(args.nEpochs_pred):
        ...
        for batch_idx, (X,Rx,Y,weight, setidx) in enumerate(Loader_train):
            ...
            YYi = Yi[:,0]
            loss = MSEloss(Y_hat.squeeze(),YYi.squeeze())
        ...
        if epoch % 1==0:
            ...
            with torch.no_grad():
                for X,Rx,Y,weight, setidx in Loader_val:
                    ...
                    YY = Y[:,0]
                    lossi = MSEloss(output.squeeze(),YY.squeeze()) 
                    loss = loss + lossi
            ...
            if loss_iepoch < loss_best:
                ...
                torch.save(f1_pred_from_Rx.state_dict(), os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_REweighted_iloop_"  + str(ithres) + '_net_Best'  + '_net.pt'))
...
file_net_dict = os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_REweighted_iloop_" +str(idx_best2) + '_net_Best' + '_net.pt')
f1_pred_from_Rx = pred_from_Rx(args.latent_dim, outdim = 1)
...
import pickle
with open(os.path.join(args.save_pickle,  "dCOR_idata_result_" + The_DATA_MARK  + "_Model_iloop_" +  str(idx_data) + '_net_Best' + '_result.pickle'), 'wb') as handle:
    pickle.dump(
        res
