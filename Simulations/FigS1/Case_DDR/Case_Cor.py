    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if dCorloss_iepoch > dCorloss_best:
                ...
                torch.save(net.state_dict(), os.path.join(args.save, "dCOR_idata_" + The_DATA_MARK  + "Model_REweighted_iloop_" + str(ithres) + '_net_Best' + '_net.pt'))
    #####Train the Repre part down
idx_best = np.argmax(error_val_dCor)
file_net_dict =  os.path.join(args.save, "dCOR_idata_" + The_DATA_MARK  + "Model_REweighted_iloop_"   + str(idx_best) + '_net_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim)
net.load_state_dict(torch.load(file_net_dict))


f1_pred_from_Rx = pred_from_Rx(args.latent_dim,outdim = 1)
opt_pred1 = optim.Adam(f1_pred_from_Rx.parameters(),lr=args.lr_pred, weight_decay=1e-4)
...
for epoch_2 in range(args.nEpochs_pred):
    ...
    for batch_idx, (X,Rx,Y, weight, setidx) in enumerate(Loader_train):
        ...
        Xi, Yi, weight = X.to(device),Y.to(device),weight.to(device)
        Rx_est,_ = net(Xi)
        Rx_est = Rx_est.detach_()
        Yhat = f1_pred_from_Rx(Rx_est)
        YY = Y[:,0]
        loss = MSEloss(Yhat.squeeze(),YY)  
        opt_pred1.zero_grad()
        loss.backward()
        opt_pred1.step()
    ########################################################
    ...
    if epoch_2 %1 ==0:
        ...
        with torch.no_grad():
            for X,Rx,Y,weight, setidx in Loader_val:
                ...
                lossi = MSEloss(output.squeeze(),YY) 
                loss = loss + lossi
        ...
        if loss_iepoch < loss_best:
            ...
            torch.save(f1_pred_from_Rx.state_dict(), os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK + "Model_REweighted_iloop_"  + str(idx_best) + '_net.pt'))
...
file_net_dict = os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_REweighted_iloop_"  + str(idx_best) + '_net.pt')
