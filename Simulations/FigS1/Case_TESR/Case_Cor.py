    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if dCorloss_iepoch < dCorloss_best:
                ...
                torch.save(net.state_dict(), os.path.join(args.save, "dCOR_" + The_DATA_MARK  + "Model_loop_" +str(ithres) + '_net_Best' + '_net.pt'))
...
idx_best = np.argmin(error_val_dCor)

file_net_dict =  os.path.join(args.save, "dCOR_" + The_DATA_MARK  + "Model_loop_"  + str(idx_best) + '_net_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim, outdim = 2)
    ...
    for epoch in range(args.nEpochs):
        ...
        if epoch % 1==0:
            ...
            if loss_iepoch < loss_best:
                ...
                torch.save(net_Target.state_dict(), os.path.join(args.save, "dCOR_net2_idata_" + The_DATA_MARK + "Model_loop_" +str(ithres) + '_net_Best' + '_net.pt'))
...
print(list_tuning[idx_best2])
file_net_dict =  os.path.join(args.save, "dCOR_net2_idata_" + The_DATA_MARK  + "Model_loop_"  + str(idx_best2) + '_net_Best' + '_net.pt')
...
net = net.eval()
net2 = net2.eval()
f1_pred_from_Rx = pred_from_Rx(args.latent_dim*2,outdim=1)
...
for epoch in range(args.nEpochs_pred):
    ...
    for batch_idx, (X,Rx,Y,weight, setidx) in enumerate(Loader_train):
        ...
        G_loss = MSEloss(Yhat.squeeze(),Yi[:,0].squeeze() )
    ...
    if epoch % 1==0:
        ...
        with torch.no_grad():
            for X,Rx,Y,weight, setidx in Loader_val:
                ...
                Yhat = f1_pred_from_Rx(Rx)
                lossi = MSEloss(Yhat.squeeze(), Y[:,0].squeeze())
                loss = loss + lossi
        loss = loss / len(Loader_val)
        loss_iepoch = loss
        if epoch %5==0:
        ...
        if loss_iepoch < loss_best:
            ...
            loss_best = loss_iepoch
            torch.save(f1_pred_from_Rx.state_dict(), os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_loop_"  + str(1) + '_f1_Best'  + '_net.pt'))


file_net_dict = os.path.join(args.save, "dCORpred_idata_" + The_DATA_MARK  + "Model_loop_"  + str(1) + '_f1_Best'  + '_net.pt')
f1_pred_from_Rx = pred_from_Rx(args.latent_dim*2,outdim =  1)
...
with torch.no_grad():
    ...
    Rx2_pred,_ = net2(x_test)
    Rx = torch.cat((Rx_pred,Rx2_pred),dim=1)
    y_pred = f1_pred_from_Rx(Rx)
