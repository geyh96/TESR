...
for i in range(3):
    for j in range(2):
        ...
        line = axs[i].errorbar(list_NT, means, yerr=stds, fmt=line_fmt[j],color=colors[j],capsize=5,label=methods[j])
...
for i in range(3):
    for j in range(2):
        ...
        means = dicts[methods[j]]["mean"][i]
        stds = dicts[methods[j]]["std"][i]
        line = axs[3+i].errorbar(list_NS, means, yerr=stds, fmt=line_fmt[j],color=colors[j],capsize=5,label=methods[j])
...
plt.tight_layout()
plt.savefig('Simulation_S3_3.pdf',dpi=500)
