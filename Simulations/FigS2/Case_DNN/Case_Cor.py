...
parser.add_argument('--iloop', type=int, default=333)
...
for ithres in range(nthres):
    ...
    for epoch in range(args.nEpochs_pred):
        ...
        if loss_iepoch < loss_best_ifold:
            ...
            torch.save(net.state_dict(), os.path.join(args.save,   The_DATA_MARK  + "Mloop_" +  str(ithres) + '_net_Best' + '_net.pt'))
##########################prediction
# 
idx_best = np.argmin(loss_eval)
file_net_dict =  os.path.join(args.save,   The_DATA_MARK  + "Mloop_" +  str(idx_best ) + '_net_Best' + '_net.pt')
net = Generator(xdim = P, ndim = args.latent_dim)
...
import pickle
with open(os.path.join(args.save_pickle,  "Result_" + The_DATA_MARK  + '_net_Best' + '.pickle'), 'wb') as handle:
